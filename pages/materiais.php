    ...
    if (!empty($nome) && !empty($categoria_id)) {
        try {
            ...
            $stmt->execute([
                ...
                'validade' => $validade ?: null, // Se a data estiver vazia, manda nulo
                    ...
                    <form method="POST" action="">
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" name="nome" class="form-control" required placeholder="Ex: Seringa 5ml">
                            </div>
                        ...
                        <button type="submit" class="btn btn-primary"><i class="bi bi-plus-circle me-1"></i> Salvar Material</button>
                    </form>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-striped mb-0">
                            ...
                            <tbody>
                                <?php if (count($materiais) > 0): ?>
                                    <?php foreach ($materiais as $mat): ?>
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($mat['categoria_nome']); ?></td>
                                            <td>
                                                <?php 
                                                // Destaca em vermelho se estiver abaixo do mínimo
                                                if ($mat['quantidade'] <= $mat['estoque_minimo']) {
                                                    echo "<span class='badge bg-danger'>{$mat['quantidade']}</span>";
                                                } else {
                                                    echo "<span class='badge bg-success'>{$mat['quantidade']}</span>";
                                                }
                                                ?>
                                            </td>
                                            <td><?php echo $mat['estoque_minimo']; ?></td>
                                            <td>
                                                <?php 
                                                if ($mat['validade']) {
                                                    echo date('d/m/Y', strtotime($mat['validade']));
                                                } else {
                                                    echo "-";
                                                }
                                                ?>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" class="text-center py-3">Nenhum material cadastrado ainda.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
